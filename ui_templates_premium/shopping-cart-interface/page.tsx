...
import {
  ShoppingCart, Trash2, Plus, Minus, Heart, CreditCard, Truck,
  Package, TrendingUp, TrendingDown, DollarSign, Tag, AlertCircle,
  CheckCircle, ArrowLeft, Search, Gift, Percent, Clock
} from 'lucide-react'

// Cart metrics with TypeScript constants
const CART_METRICS = [
  {
    id: 'total_items',
    label: 'Total Items',
...
export default function ShoppingCartInterface() {
          ...
          <div className='flex items-center justify-between'>
            <div className='flex items-center space-x-4'>
              ...
              <div>
                ...
                <p className='text-gray-600'>{cartSummary.itemCount} items in your cart</p>
              </div>
            </div>
              ...
              <CardContent className='h-80'>
                <ResponsiveContainer width='100%' height='100%'>
                  <BarChart data={SPENDING_DATA}>
                    ...
                    <Legend />
                    <Bar dataKey='amount' fill='#10b981' name='Spending' radius={[4, 4, 0, 0]} />
            ...
            <Card className='border border-gray-200 shadow-sm h-full'>
              ...
              <CardContent className='space-y-4'>
                ...
                <div className='flex justify-between text-lg'>
                  ...
                  <span className='font-bold text-emerald-600'>${cartSummary.total.toFixed(2)}</span>
                </div>
          ...
          <Card className='border border-gray-200 shadow-sm'>
            ...
            <CardContent>
              <div className='space-y-4'>
                <AnimatePresence>
                  {cartItems.map((item) => (
                    <motion.div key={item.id} layoutId={`item-${item.id}`} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className={`p-4 bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-xl hover:shadow-md transition-all ${selectedItem === item.id ? 'ring-2 ring-emerald-500' : ''}`}>
                      <div className='flex items-center space-x-4'>
                        ...
                        <div className='flex-1'>
                          ...
                          <div className='flex items-center space-x-2 mt-2'>
                            <span className='text-xl font-bold text-emerald-600'>${(item.price * item.quantity).toFixed(2)}</span>
                ...
                {RECOMMENDED_ITEMS.map((item) => (
                  <motion.div key={item.id} whileHover={{ y: -4 }} className='p-4 bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-xl'>
                    <div className='text-center'>
                      ...
                      <div className='text-lg font-bold text-emerald-600 mt-2'>${item.price}</div>
                      <Button size='sm' className='mt-3 w-full bg-gradient-to-r from-emerald-600 to-teal-600'><Plus className='w-4 h-4 mr-2' />Add to Cart</Button>
                    </div>
                  </motion.div>
                ))}
              </div>
